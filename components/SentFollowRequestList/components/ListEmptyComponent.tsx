...
type Props = {
  ...
  ) => Promise<QueryObserverResult<InfiniteData<ListSentFollowRequestsResponse, unknown>, Error>>;
};
...
const ListEmptyComponent = ({ isLoading, isError, refetch }: Props) => {
  ...
  if (isError) {
    return (
      <ErrorMessageWithRefresh refresh={refetch} errorText="There was an error fetching your sent follow requests" />
    );
  }
