...
const ProfileReportDetailScreen = () => {
  const { id } = useLocalSearchParams<{ id: string }>();
  const tabBarHeight = useBottomTabBarHeight();
  const { setLightOrDark } = useColorMode();

  const fetchReport = async () => {
    const res = await getProfileReport(id);
    return res.data;
  };

  const report = useQuery({
    queryKey: queryKeys.profileReports.details(id),
    queryFn: fetchReport,
  ...
  return (
    ...
    >
      <View style={s.headerRow}>
        <Text style={s.username}>@{data.profile_username}</Text>
...
export default ProfileReportDetailScreen;
