...
import { getProfileReportsForQuery } from "@/api/profile-report";
...
const ReportItem = ({ item, onPress }: { item: ProfileReport; onPress: () => void }) => {
...
const ProfileReportsScreen = () => {
  const tabBarHeight = useBottomTabBarHeight();
  const router = useRouter();

  const fetchReports = async ({ pageParam }: { pageParam: string }) => {
    const res = await getProfileReportsForQuery(pageParam);
    return res.data;
  };

  const reports = useInfiniteQuery({
    queryKey: queryKeys.profileReports.root,
  ...
  return (
    <FlashList
      ...
      renderItem={({ item }) => (
        <ReportItem item={item} onPress={() => router.push(`/(app)/profile/profileReports/${item.id}`)} />
      )}
      ListEmptyComponent={
...
export default ProfileReportsScreen;
